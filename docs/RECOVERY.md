...
Redrob Recall stores derived data in `~/.redrob/recall`. Original files remain authoritative and are never modified by backup, recovery, or index-clear operations.
...
- The three newest automatic or manual metadata backups are retained under `~/.redrob/recall/backups`.
...
1. Quit Redrob Recall completely.
2. Copy `~/.redrob/recall` to a separate safe location.
3. Choose a backup from `~/.redrob/recall/backups`.
4. Move `metadata.db`, `metadata.db-wal`, and `metadata.db-shm` out of the data directory if present.
5. Copy the chosen backup to `~/.redrob/recall/metadata.db`.
6. Remove or rename `~/.redrob/recall/qdrant-edge` so semantic vectors are rebuilt.
7. Start Redrob Recall and select **Check now**.
